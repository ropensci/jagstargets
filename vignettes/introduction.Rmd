...
Consider the jags model file below.
...
`jagstargets` encapsulates this workflow with the [`tar_jags()`](https://wlandau.github.io/jagstargets/reference/tar_jags.html) function. To use it in a [`targets`](https://docs.ropensci.org/targets/) pipeline, invoke it from the `_targets.R` script of the project.
...
[`tar_jags()`](https://wlandau.github.io/jagstargets/reference/tar_jags.html) only *defines* the pipeline. It does not actually run JAGS, it declares the targets that will eventually run JAGS. The specific targets are as follows. Run `tar_manifest()` to show specific details about the targets declared.
